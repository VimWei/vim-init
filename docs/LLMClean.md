...
6. `gv<Plug>MarkdownRemoveAll`（选区）或 `ggVG<Plug>MarkdownRemoveAll`（全文） — 移除所有 Markdown 文本格式（粗体、斜体、删除线、行内代码等）。仅在 `filetype=markdown` 时执行，否则静默跳过。
...
- `s:ExecuteItems(items, range, start_line, end_line)` — 按 `s:exec_order` 顺序执行选中的操作
  ...
  - 第6项特殊处理：检查 `&filetype == 'markdown'`，选区模式通过具体行号选中，全文模式通过 `normal! ggVG` 全选，后执行 `<Plug>MarkdownRemoveAll`，非 markdown 文件类型时静默跳过
